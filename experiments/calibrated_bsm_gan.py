...
"""Train an SDE as a GAN, on data from a Black Scholes SDE which is mean square stable, calibrated.
- normalization applied
- uniform distribution as initial condition

To run, execute:
python -m experiments.calibrated_bsm_gan
"""
import fire
...
def get_data(batch_size, device):
    ...
    ou_sde = BsmSDE(mu=0.00, theta=0.5/100, sigma=sigma).to(device)
